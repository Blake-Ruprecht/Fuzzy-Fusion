...
class FuzzyNeuron(torch.nn.Module):
    ...
    def forward(self,input):
        ...
        ants = torch.zeros( self.R, batchSize, self.A )                         # our rule antecedents
...
if __name__=='__main__':
    ...
    # Synthetic Experimental Controls ------------------------------------------
        # Just modify these parameters for the synthetic experiment, don't worry
        # about everything below this.

    # in the synthetic data...
    torch.manual_seed( 4242 )
    NoSamples = 700                                                             # this is the number of SAMPLES
    NoPatterns = 10                                                             # this is the number of PATTERNS

    # in the ANFIS Neuron...
    NoRules = 10                                                                # this is the number of RULES to "guess"
    NoAnts = 2                                                                  # this is the number of ANTECEDENTS, which needs to be correct
    initMethod = 2                                                              # this is the INITIALIZATION METHOD, 0 = random, 1 = k-means, 2 = k-means w/rho

    # training...
    NoEpochs = 10                                                               # this is the number of EPOCHS to run
    ...
    df.to_csv('clusterData_anfis.csv', index=False, header=False )
    ...
    net = FuzzyNeuron(NoRules,NoAnts,initMethod,x,l)                            # create the ANFIS neuron
